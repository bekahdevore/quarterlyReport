...
###################################### JOB DESCRIPTIONS        
############################ IT
itBachelorsInfo    <- " Information Technology Managers; Systems Analysts; Operations Analysts"
itBachelorsProg    <- " Software Engineers; Systems Engineers"
itBachelorsNet     <- " Network Specialists; Database Administrators; Information Security Analysts; Systems Administrators"
itBachelorsWeb     <- " Computer Programmers; Graphic Designers"
itAssociatesInfo   <- " Technical Support Analysts; Information Technology Support Technicians"
...
############################ HEALTHCARE
healthDoctoralDire    <- "Physicians and Surgeons; Physical Therapists; Audiologists; Family and General Practitioners"                                       ## DOCTORAL 
healthDoctoralDiag    <- "Dentists; Orthodontists; Pharmacists; Optometrists"
healthDoctoralAdmi    <- "Healthcare Lawyers"
healthMastersDire     <- "Nurse Practitioners; Occupational Therapists; Physicians Assistants; Nurse Anesthetists; Speech-Language Pathologists"              ## MASTERS
healthMastersDiag     <- "Mental Health Counselors"
healthMastersAdmi     <- "Healthcare Social Workers; Statisticians"                                                                                           ## BACHELORS
...
healthAssociatesDire  <- "Physical and Occupational Therapist Assistants; Respiratory Therapists; Sonographers; Cardiovascular Technicians"                    ## ASSOCIATES
...
healthCertificateDire <- "Nursing Assistants; Emergency Technicians and Paramedics; Medical Assistants; Licensed Nurses"                                       ## CERTIFICATE
...
healthHighSchoolDire  <- "Physical Therapists Aides; Patient Transporters; Sterile Processing Technicians; Endoscopy Technicians; Emergency Room Technicians"  ## HIGH SCHOOL
...
################# JOB POSTINGS
####### TOTALS           
totalJobsLog           <-  sumDeduplicatedPostings(logJobs)
...
## EDUCATION LEVEL
# BACHELORS
logWagesBaProLOW  <- wages25(proBa$Pct..25.Hourly.Earnings)
...
################# JOB POSTINGS
####### TOTALS           
totalJobsMan           <-  sumDeduplicatedPostings(manJobs)
...
################# JOB POSTINGS
####### TOTALS           
totalJobsBus           <-  sumDeduplicatedPostings(busJobs)
...
# ASSOCIATES
busWagesAsFinLOW <- ""
busWagesAsLegLOW <- roundMean(legAs$Pct..25.Hourly.Earnings)
busWagesAsAdvLOW <- roundMean(advAs$Pct..25.Hourly.Earnings)
# CERTIFICATES 
busWagesCeFinLOW <- ""
busWagesCeLegLOW <- roundMean(legCe$Pct..25.Hourly.Earnings)
busWagesCeAdvLOW <- roundMean(advCe$Pct..25.Hourly.Earnings)
# HIGH SCHOOL 
...
## ASSOCIATES
busWagesAsFinHIGH <- ""
busWagesAsLegHIGH <- roundMean(legAs$Pct..75.Hourly.Earnings)
busWagesAsAdvHIGH <- roundMean(advAs$Pct..75.Hourly.Earnings)
## CERTIFICATES
busWagesCeFinHIGH <- ""
busWagesCeLegHIGH <- roundMean(legCe$Pct..75.Hourly.Earnings)
busWagesCeAdvHIGH <- roundMean(advCe$Pct..75.Hourly.Earnings)
## HIGH SCHOOL 
...
################# JOB POSTINGS
####### TOTALS           
totalJobsFood           <-  sumDeduplicatedPostings(foodJobs)
...
################# JOB POSTINGS
####### TOTALS           
totalJobsHealth           <-  sumDeduplicatedPostings(healthJobs)
totalJobPostingsHealthDo  <-  sumDeduplicatedPostings(healthJobsDo)
